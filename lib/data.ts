...
/**
 * SiteIcon auto-fetches logos from Google Favicon API (sz=256) at runtime.
 * The `faviconUrl` field is optional — leaving it undefined lets SiteIcon resolve
 * the best available logo automatically using the site's domain.
 */
const RAW_SITES: Omit<Site, 'order'>[] = [
  ...
  {
    id: 'mruxjgyctzacj', name: 'world4ufree', url: 'https://world4ufree.im/', domain: 'world4ufree.im',
    category: 'Movies & Shows', regions: ['IN'], tags: ['trusted'],
    isTrusted: true, isNew: false, isFeatured: false,
    description: 'Bollywood, Hollywood, and South Indian movies download site.',
    addedAt: d(1),
  },
...
// Backfill `order` per category sequentially so every site starts with a
// distinct, predictable 0-indexed position within its category.
export const SITES: Site[] = (() => {
  const counters: Record<string, number> = {};
  return RAW_SITES.map(s => {
    const currentOrder = counters[s.category] ?? 0;
    counters[s.category] = currentOrder + 1;

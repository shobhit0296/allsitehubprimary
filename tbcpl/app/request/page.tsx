...
export default function RequestPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!siteUrl.trim() || !siteName.trim()) { setError('URL aur naam zaroor bharo.'); return; }
    setSubmitting(true);
    setError('');
    try {
      const res = await fetch('/api/requests', {
        ...
        body: JSON.stringify({ siteUrl, siteName, targets, reason }),
      });
      if (res.ok) {
        setDone(true);
      } else {
  ...
  return (
    <div className="min-h-screen flex flex-col relative">
      ...
      {/* Hero */}
      <section className="relative overflow-hidden text-center px-4 pt-14 pb-2">
          ...
          <div className="glass-lux border border-emerald-500/25 rounded-2xl p-10 text-center">
            ...
            <div className="flex gap-2.5 justify-center flex-wrap">
              <button
                ...
                className="px-5 py-2.5 bg-gradient-to-r from-blue-500 to-violet-500 rounded-xl text-white text-sm font-bold"
              >
                Submit Another
              </button>
          ...
          <div className="glass-lux border border-white/10 rounded-2xl p-6 sm:p-7 shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4.5">
              ...
              <button
                type="submit"
                disabled={submitting}
                className={`flex items-center justify-center gap-2 rounded-xl px-6 py-3 text-white text-sm font-bold transition-all ${
                  submitting
                    ? 'bg-blue-500/50 cursor-not-allowed'
                    : 'bg-gradient-to-r from-blue-500 to-violet-500 shadow-lg shadow-blue-500/20 hover:scale-[1.02]'
                }`}
              >
                {submitting ? (
                  <>

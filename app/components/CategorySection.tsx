import type { Site, Category } from '@/lib/data';
import { categoryAccent } from '@/lib/theme';
import SiteCard from './SiteCard';
import CategoryIcon from './CategoryIcon';

interface CategorySectionProps {
  category: Category;
  sites: Site[];
  bookmarks: Set<string>;
  onToggleBookmark: (id: string) => void;
}

export default function CategorySection({ category, sites, bookmarks, onToggleBookmark }: CategorySectionProps) {
  if (sites.length === 0) return null;

  const accent = categoryAccent(category.name);

  return (
    <section
      id={`cat-${category.name.replace(/\s+/g, '-').replace(/&/g, 'and').toLowerCase()}`}
      className="mb-8 sm:mb-14 fade-up transform-gpu"
      style={{
        contentVisibility: 'auto',
        containIntrinsicSize: '0 320px',
        scrollMarginTop: 'calc(var(--nav-h) + var(--cat-bar-h) + 12px)',
      }}
    >
      {/* ── Section Header ─────────────────────────── */}
      <div className="flex items-end justify-between mb-4 sm:mb-6 flex-wrap gap-2">
        <div className="flex items-center gap-3 sm:gap-4 group">
          {/* Glowing accent bar */}
          <div
            className="w-[3.5px] sm:w-[4.5px] h-9 sm:h-12 rounded-full shrink-0 transition-all duration-300 group-hover:scale-y-110"
            style={{
              background: `linear-gradient(180deg, ${accent}, ${accent}44)`,
              boxShadow: `0 0 14px ${accent}55`,
            }}
          />
          <div>
            <div className="flex items-center gap-2 sm:gap-3 mb-0.5 sm:mb-1">
              <CategoryIcon name={category.name} size={22} className="shrink-0 transition-transform duration-300 group-hover:scale-110" />
              <h2 className="font-headline text-[1.1rem] sm:text-[1.55rem] font-bold tracking-[-0.02em] text-[var(--text-primary)]">
                {category.name}
              </h2>
              <span
                className="text-[10px] sm:text-[12px] font-bold rounded-md px-2 sm:px-2.5 py-0.5 tabular-nums transition-all duration-300"
                style={{ color: accent, background: `${accent}1a`, border: `1px solid ${accent}38` }}
              >
                {sites.length}
              </span>
            </div>
            <p className="text-[12px] sm:text-[14px] text-[var(--text-muted)] leading-snug line-clamp-1 sm:line-clamp-none">
              {category.description}
            </p>
          </div>
        </div>
      </div>

      {/* ── Sites Grid: 1 xs / 2 sm / 3 tablet / 4 md / 5 lg / 6 xl ── */}
      <div className="sites-grid">
        {sites.map(site => (
          <SiteCard
            key={site.id}
            site={site}
            isBookmarked={bookmarks.has(site.id)}
            onToggleBookmark={onToggleBookmark}
          />
        ))}
      </div>
    </section>
  );
}

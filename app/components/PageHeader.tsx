  ...
  return (
    <header className={`site-header${scrolled || menuOpen ? ' is-scrolled' : ''}`}>
      <div className="nav-inner">
        <Link href="/" className="flex items-center gap-2 shrink-0 min-w-0 transition-opacity duration-300 hover:opacity-90">
          <img
            src="/icon.svg"
            alt="AllSiteHub"
            width={28}
            height={28}
            className="w-7 h-7 rounded-md object-contain bg-[#05070d] shrink-0"
          />
          <span className="font-headline nav-brand-text font-extrabold tracking-tight text-[var(--text-primary)]">
        ...
        <nav className="hidden md:flex items-center gap-3 lg:gap-5 shrink-0">
          {NAV_LINKS.map(([label, href]) => (
            <Link
              key={label}
              href={href}
              className={`nav-link-desk${label === active ? ' is-active' : ''}`}
            >
              {label}
            </Link>
          ))}
        ...
        <div className="hidden md:flex items-center gap-1.5 lg:gap-2 shrink-0">
          <a
            ...
            className="nav-icon-btn hover:!border-[#5865F2]/50 hover:!bg-[#5865F2]/10"
          >
            <DiscordIcon />
          </a>
          <Link

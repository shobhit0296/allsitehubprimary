...
export default function AdminDashboard({ panel, initialSites, initialRequests, categories, regions }: Props) {
  ...
  const handleRefreshLogo = async () => {
    ...
    const domain = form.domain.trim() || (() => { try { return new URL(form.url.trim()).hostname.replace(/^www\./, ''); } catch { return ''; } })();
    if (!domain) { setFormError('Enter a domain or URL first.'); return; }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormError('Logo refresh failed. Try again.');
      }
    } catch { setFormError('Network error during logo refresh.'); }
    finally { setRefreshingLogoId(null); }
  };

  /* ── Refresh all logos ── */
  const handleRefreshAllLogos = async () => {
    if (!confirm('This will re-fetch logos for ALL sites. It may take a minute. Continue?')) return;
    ...
    } catch { setRefreshAllResult('✗ Network error'); }
  ...
  const approveAndEdit = (req: SiteRequest) => {
    ...
    try { domain = new URL(req.siteUrl).hostname.replace(/^www\./, ''); } catch { domain = req.siteUrl; }

    setForm({
      name: req.siteName,
      url: req.siteUrl,
      description: req.reason || `${req.siteName} streaming platform.`,
      domain: domain,
      faviconUrl: '',
      category: req.targets[0]?.category || categories[0] || 'Movies & Shows',
      regions: req.targets.map(t => t.region).filter(Boolean).length > 0 ? Array.from(new Set(req.targets.map(t => t.region))) : ['Global'],
      isTrusted: false,
      isNew: true,
      isFeatured: false,
    });
    ...
    setFormError('');
    setModalMode('add');
    // Mark request approved in background
    updateReqStatus(req.id, 'approved');
  };
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 3 }}>
                <button
                  ...
                  title="Re-fetch logos for all sites from their websites"
                  style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(255,255,255,0.04)', border: '1px solid var(--border)', borderRadius: 10, padding: '9px 16px', color: 'var(--text-secondary)', fontSize: 13, fontWeight: 600, cursor: refreshAllBusy ? 'not-allowed' : 'pointer', opacity: refreshAllBusy ? 0.6 : 1, transition: 'all 0.15s', whiteSpace: 'nowrap' }}
                ...
                >
                  <span style={{ fontSize: 15, display: 'inline-block', animation: refreshAllBusy ? 'spin 1s linear infinite' : 'none' }}>🔄</span>
                  {refreshAllBusy ? 'Refreshing…' : 'Refresh All Logos'}
                </button>
                {refreshAllResult && (
                  <span style={{ fontSize: 11, color: refreshAllResult.startsWith('✓') ? 'var(--green)' : 'var(--red)', fontWeight: 600 }}>{refreshAllResult}</span>
                )}
              </div>
              {/* Add Site */}
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  <div style={{ width: 36, height: 36, borderRadius: 8, background: 'var(--bg-base)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden' }}>
                    {form.faviconUrl?.trim() ? (
                      ...
                      <img src={form.faviconUrl.trim()} alt="preview" width={24} height={24} style={{ objectFit: 'contain' }}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
                  <button
                    ...
                    style={{
                      ...
                      opacity: refreshingLogoId ? 0.6 : 1,
                      transition: 'all 0.15s',
                    }}
                    onMouseEnter={e => { if (!refreshingLogoId) (e.currentTarget as HTMLElement).style.background = 'rgba(99,102,241,0.2)'; }}
                  ...
                  >
                    ...
                    {refreshingLogoId ? 'Fetching…' : '🔄 Refresh Logo'}
                  </button>
                  <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>Auto-detects from website. Or paste a custom URL above.</span>
                </div>
